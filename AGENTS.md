...
| # | Trap | Guard |
|---|---|---|
| 14 | The bar glyph (`\uF2DB`, a Nerd Font microchip) sits in the Unicode Private Use Area. A PUA character does not survive every editing path — this project shipped a `Panel.qml` whose `barIcon` had silently become an empty string. The result: the file parsed, the component instantiated, its IPC handler registered, nothing logged a warning, and 131 tests passed, because nothing asserted the string was non-empty. The bar widget was simply invisible. | `BarGlyphTest` in `tests/test_cross_language.py` now asserts the exact escape (`literal == BAR_GLYPH`), not just its shape — the shape-only check used to pass just as happily on a typo'd codepoint as on the real glyph. Always write the glyph as a `\uXXXX` escape; never paste the literal character into either file. |
