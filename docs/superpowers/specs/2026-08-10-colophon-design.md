...
| polkit `manage-unit-files` | separate action, no `unit` detail from systemd | enable/disable **cannot** be scoped to one unit by a polkit rule. This removed the boot toggle from the MVP. **Corrected 2026-08-12:** that scoping gap is no longer blocking — see `AGENTS.md` trap #28. It stopped mattering once the privilege mechanism it was scoping (the polkit rule) stopped existing; prompted authentication asks Omarchy's own agent every time and needs no rule scoped to anything. The boot toggle is a follow-up, not a permanent limitation. **Landed, later the same day (2026-08-12):** it shipped — see `docs/superpowers/specs/2026-08-12-boot-toggle-design.md` |
...
**Landed 2026-08-12.** That follow-up has shipped — see
`docs/superpowers/specs/2026-08-12-boot-toggle-design.md` and `AGENTS.md`
trap #28. Nothing about the boot toggle is pending any more.
