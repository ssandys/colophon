...
- **Python is stdlib-only, no pip ever.** Permitted imports across both scripts: `json`, `os`, `sys`, `re`, `glob`, `time`, `shlex`, `subprocess`, `shutil`, `urllib.request`, `urllib.error`, `datetime`.
