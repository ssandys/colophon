...
```bash
./bin/install
omarchy bar move ssandys.colophon-dev --section right
omarchy restart shell
```

Expected: a microchip glyph in the right section of the bar. Clicking it opens a panel containing only the title `  Colophon`. If the glyph is missing, run `omarchy-shell shell rescanPlugins` and check `~/.config/omarchy/plugins/ssandys.colophon-dev/` exists.
...
3. **Install**: `omarchy plugin add https://github.com/ssandys/colophon.git --enable`, then `omarchy bar move ssandys.colophon --section right`. Then the privilege step, with both paths: `sudo bin/install-privileges` from a published clone at `~/.config/omarchy/plugins/ssandys.colophon/`, or from your checkout for a dev install. Show exactly what the grant permits — one user, three verbs, one unit — and how to check (`bin/install-privileges --check`) and remove it (`--remove`).
