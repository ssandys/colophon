...
Item {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      // isTransient, not transient: `transient` is reserved in QML's JS
      // grammar. QQmlJS still reserves the ECMAScript 3 future-reserved words,
      // so `volatile`, `synchronized`, `native`, `throws`, `goto`, and
      // `implements` are out too, as identifiers AND as property names.
...
3. **Traps table**, each row naming its guard test. Seed it with the four carried from galley — #10 `Process` spawn failure (guarded by the `onRunningChanged` handlers in `Service.qml`), #11 `Process.command` assignment while running (the single pending-notification slot), #12 cross-language duplication (`tests/test_cross_language.py`), #13 caller-owned diff state (`expectedStop` in `Service.qml`) — plus Colophon's own: `manage-unit-files` cannot be scoped by polkit; embedding models need `/api/embed`; installed totals sum unique digests while rows sum layers; `transient` is reserved in QML's JS grammar -- QQmlJS still reserves the ECMAScript 3 future-reserved words (`volatile`, `synchronized`, `native`, `throws`, `goto`, `implements` too), as identifiers and as property names; **qmllint 1.0 here reports every parse error as a bare exit 255 with no message at all**, so the QML gate tells you a file failed to parse but never why -- bisect by commenting out blocks, or reach for `qmlformat`; `MemoryCurrent` has two "unknown" sentinels; `ExecMainStartTimestamp` is locale-dependent so the monotonic variant is used; nanosecond fractions break `fromisoformat`; SI vs binary bytes; and the unit's `Restart=on-failure`/`RestartSec=3` means a crash can open and close a `failed` window narrower than the poll interval, so a short-lived death may never be reported — the gap phase-2's D-Bus subscription closes.
