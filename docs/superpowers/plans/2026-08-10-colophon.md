...
- **Standing safety rule:** no test, script, or manual check may start, stop, or restart the real `ollama.service`, and none may pull or delete a model. Every privileged verb is asserted through `--dry-run` only. The one exception is Task 2/3, which start the service by hand, once, deliberately.
...
curl -fsSL https://raw.githubusercontent.com/ssandys/galley/main/LICENSE -o LICENSE 2>/dev/null \
